...
namespace cli
{
    ...
    {
        public:
            explicit Command(std::string _name, std::vector<std::string> _options = {}) : name(std::move(_name)), enabled(true), options(_options) {}
            ...
            std::vector<std::string> GetOptionCompletions(const std::string& line)
            {
                std::vector<std::string> r;
                std::string p = line.substr(line.rfind(' ') + 1);
    ...
    class CliSession
    {
        public:
            ...
            std::vector<std::string> GetOptionCompletions(
    ...
    class Menu : public Command
    {
        public:
            ...
            template <typename F>
                CmdHandler Insert(const std::string& cmdName, const std::vector<std::string>& parDesc, F f, const std::string& help, const std::vector<std::string>& options)
                {
                    // dispatch to private Insert methods
                    return Insert(cmdName, help, parDesc, f, &F::operator(), options);
                }
            ...
            std::vector<std::string> GetOptionCompletions(const std::string& line)
            {
                ...
                {
                    assert(0 != cmd);
                    auto cs = cmd->GetOptionCompletions(line);
                    if (!cs.empty()) {
                        return cs;
                    }
                }
        ...
        private:
            ...
            template <typename F, typename R>
                CmdHandler Insert(const std::string& name, const std::string& help, const std::vector<std::string>& parDesc, F& f, R (F::*)(std::ostream& out, std::vector<std::string>) const, const std::vector<std::string>& options);
        ...
        class VariadicFunctionCommand : public Command
    {
        public:
            ...
            VariadicFunctionCommand(
                ...
                : Command(_name, options), func(std::move(fun)), description(std::move(desc)), parameterDesc(std::move(parDesc))
            {
            }
    ...
    inline std::vector<std::string> CliSession::GetOptionCompletions(
            std::string currentLine) const
    {
        return current->GetOptionCompletions(currentLine);
    }
    ...
    template <typename F, typename R>
        CmdHandler Menu::Insert(const std::string& cmdName, const std::string& help, const std::vector<std::string>& parDesc, F& f, R (F::*)(std::ostream& out, std::vector<std::string> args) const, const std::vector<std::string>& options)
        {
            return Insert(std::make_unique<FreeformCommand<F>>(cmdName, f, help, parDesc, options));
        }
